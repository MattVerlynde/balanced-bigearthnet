...
def create_split(root_folder, patch_names, labels_patch_names, TFRecord_writer, label_indices, GDAL_EXISTED, RASTERIO_EXISTED):
    if GDAL_EXISTED:
        import gdal
    ...
    for patch_idx, patch_name in enumerate(patch_names):
        patch_folder_path = os.path.join(root_folder, patch_name[:44], patch_name)
        bands = {}
        ...
        original_labels = labels_patch_names[patch_idx]
        # print("len original_labels is" + str(len(original_labels)))
...
def prep_tf_record_files(root_folder, out_folder, split_names, patch_names_list, labels_patch_names_list, label_indices, GDAL_EXISTED, RASTERIO_EXISTED):
    try:
        writer_list = []
    ...
    for split_idx in range(len(patch_names_list)):
        ...
        create_split(
            ...
            labels_patch_names_list[split_idx],
